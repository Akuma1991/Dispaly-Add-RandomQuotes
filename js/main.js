...
var addQuoteContent = document.getElementById('addQuoteContent');
var addQuoteAuthor = document.getElementById('addQuoteAuthor');
var invalidQuote = document.getElementById('invalidQuote');
// var quotes = [
...
var color = ['#FF6633', '#FFB399', '#FF33FF', '#00B3E6',
...
if (localStorage.getItem('allQuotes') == null) {
    ...
    var quotes = JSON.parse(localStorage.getItem('allQuotes'));
...
function addQuote() {
    var quoteObject = {
        quoteContent: addQuoteContent.value,
    ...
    if (quoteObject.quoteAuthor == "" || quoteObject.quoteContent == "") {
        ...
        invalidQuote.innerHTML = `This quote already exists`;

    }
    else {
        quotes.push(quoteObject);
        localStorage.setItem('allQuotes', JSON.stringify(quotes));
        invalidQuote.innerHTML = ``;
...
function compareQuotes() {
    for (var i = 0; i < quotes.length; i++) {
        if (quotes[i].quoteContent.toLocaleLowerCase() == addQuoteContent.value.toLocaleLowerCase())
            return 1;
